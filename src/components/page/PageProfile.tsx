...
const PageProfile: React.FC<PageProfileProps> = ({ pageId, pageData }) => {
  ...
  const fetchPageDetails = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch page details"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const checkPageMembership = async () => {
    try {
      ...
      const isMember = members.some(
        (member: any) => member.user._id === profile._id
      );
  ...
  useEffect(() => {
    ...
    followersObserver.current = new IntersectionObserver(
      (entries) => {
        if (
          entries[0].isIntersecting &&
          currentFollowersPage < totalFollowersPages - 1 &&
          !isLoadingFollowers
        ) {
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="space-y-4">
                  ...
                  {currentFollowersPage >= totalFollowersPages - 1 &&
                    followers.length > 0 && (
                      <div className="text-center py-4 text-gray-500">
                        Đã hiển thị tất cả người theo dõi ({totalFollowersPages}{" "}
                        trang)
                      </div>
                    )}
