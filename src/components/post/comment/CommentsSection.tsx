...
const CommentsSection = ({ postId, totalComments, onRefresh }: any) => {
  ...
  const fetchComments = useCallback(
        ...
        if (response.result) {
          ...
          setCommentState((prev) => ({
            ...
            parentComments:
              page === 0
                ? newComments
                : [...prev.parentComments, ...newComments],
    ...
    [
      commentState.hasMoreParents,
      commentState.loading,
      commentState.parentComments.length,
      get,
      postId,
    ]
  );
  ...
  const fetchChildComments = useCallback(
    async (parentId: string, page: number) => {
      setLoadingStates((prev) => ({ ...prev, [parentId]: true }));
      console.log(
        "Fetching child comments for parentId:",
        parentId,
        "Page:",
        page
      );
                ...
                <>
                  ...
                  {commentState.childCommentsMap[comment._id] && (
                    <div className="ml-8 space-y-2">
                      {commentState.childCommentsMap[comment._id].map(
                        (childComment, childIndex) => (
                          <CommentItem
                            key={`child-${childComment._id}-${childIndex}`}
